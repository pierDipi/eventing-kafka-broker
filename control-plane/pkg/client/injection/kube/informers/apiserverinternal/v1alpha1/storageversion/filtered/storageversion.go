...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Internal().V1alpha1().StorageVersions()
...
func Get(ctx context.Context, selector string) v1alpha1.StorageVersionInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/apiserverinternal/v1alpha1.StorageVersionInformer with selector %s from context.", selector)
	}
	return untyped.(v1alpha1.StorageVersionInformer)
}
...
var _ apiserverinternalv1alpha1.StorageVersionLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apiapiserverinternalv1alpha1.StorageVersion{}, 0, nil)
}

func (w *wrapper) Lister() apiserverinternalv1alpha1.StorageVersionLister {
	return w
}
