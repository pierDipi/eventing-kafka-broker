...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Apps().V1beta2().Deployments()
...
func Get(ctx context.Context, selector string) v1beta2.DeploymentInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/apps/v1beta2.DeploymentInformer with selector %s from context.", selector)
	}
	return untyped.(v1beta2.DeploymentInformer)
}
...
var _ appsv1beta2.DeploymentLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apiappsv1beta2.Deployment{}, 0, nil)
}

func (w *wrapper) Lister() appsv1beta2.DeploymentLister {
	return w
}
