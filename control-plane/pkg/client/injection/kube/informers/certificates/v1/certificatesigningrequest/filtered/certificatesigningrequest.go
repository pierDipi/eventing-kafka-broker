...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Certificates().V1().CertificateSigningRequests()
...
func Get(ctx context.Context, selector string) v1.CertificateSigningRequestInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/certificates/v1.CertificateSigningRequestInformer with selector %s from context.", selector)
	}
	return untyped.(v1.CertificateSigningRequestInformer)
}
...
var _ certificatesv1.CertificateSigningRequestLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apicertificatesv1.CertificateSigningRequest{}, 0, nil)
}

func (w *wrapper) Lister() certificatesv1.CertificateSigningRequestLister {
	return w
}
