...
func Get(ctx context.Context, selector string) v1.EndpointSliceInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/discovery/v1.EndpointSliceInformer with selector %s from context.", selector)
	}
...
var _ discoveryv1.EndpointSliceLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apidiscoveryv1.EndpointSlice{}, 0, nil)
}

func (w *wrapper) Lister() discoveryv1.EndpointSliceLister {
	return w
}
