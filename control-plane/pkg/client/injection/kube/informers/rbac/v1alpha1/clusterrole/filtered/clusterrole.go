...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Rbac().V1alpha1().ClusterRoles()
...
func Get(ctx context.Context, selector string) v1alpha1.ClusterRoleInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/rbac/v1alpha1.ClusterRoleInformer with selector %s from context.", selector)
	}
	return untyped.(v1alpha1.ClusterRoleInformer)
}
...
var _ rbacv1alpha1.ClusterRoleLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apirbacv1alpha1.ClusterRole{}, 0, nil)
}

func (w *wrapper) Lister() rbacv1alpha1.ClusterRoleLister {
	return w
}
