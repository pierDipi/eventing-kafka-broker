...
func Get(ctx context.Context, selector string) v1.PriorityClassInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/scheduling/v1.PriorityClassInformer with selector %s from context.", selector)
	}
