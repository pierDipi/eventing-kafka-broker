...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Scheduling().V1alpha1().PriorityClasses()
...
func Get(ctx context.Context, selector string) v1alpha1.PriorityClassInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/scheduling/v1alpha1.PriorityClassInformer with selector %s from context.", selector)
	}
	return untyped.(v1alpha1.PriorityClassInformer)
}
...
var _ schedulingv1alpha1.PriorityClassLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apischedulingv1alpha1.PriorityClass{}, 0, nil)
}

func (w *wrapper) Lister() schedulingv1alpha1.PriorityClassLister {
	return w
}
