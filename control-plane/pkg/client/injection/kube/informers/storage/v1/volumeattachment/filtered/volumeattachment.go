...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Storage().V1().VolumeAttachments()
...
func Get(ctx context.Context, selector string) v1.VolumeAttachmentInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch k8s.io/client-go/informers/storage/v1.VolumeAttachmentInformer with selector %s from context.", selector)
	}
	return untyped.(v1.VolumeAttachmentInformer)
}
...
var _ storagev1.VolumeAttachmentLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apistoragev1.VolumeAttachment{}, 0, nil)
}

func (w *wrapper) Lister() storagev1.VolumeAttachmentLister {
	return w
}
