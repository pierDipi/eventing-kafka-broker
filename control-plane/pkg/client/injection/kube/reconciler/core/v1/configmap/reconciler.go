...
// controller reconciling v1.ConfigMap.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.ConfigMap. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.ConfigMap) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.ConfigMap.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.ConfigMap. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.ConfigMap) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.ConfigMap if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.ConfigMap.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.ConfigMap) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.ConfigMap if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.ConfigMap.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.ConfigMap) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.ConfigMap) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.ConfigMap resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client kubernetes.Interface, lister corev1.ConfigMapLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ConfigMaps(s.namespace)
...
// func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.ConfigMap, desired *v1.ConfigMap) error {
// 	existing = existing.DeepCopy()
// 	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
// 		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
// 		if attempts > 0 {
//
// 			getter := r.Client.CoreV1().ConfigMaps(desired.Namespace)
//
...
//
// 		updater := r.Client.CoreV1().ConfigMaps(existing.Namespace)
//
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.ConfigMap) (*v1.ConfigMap, error) {
	...
	patcher := r.Client.CoreV1().ConfigMaps(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.ConfigMap) (*v1.ConfigMap, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.ConfigMap, reconcileEvent reconciler.Event) (*v1.ConfigMap, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
