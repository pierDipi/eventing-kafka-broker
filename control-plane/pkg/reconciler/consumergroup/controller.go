...
const (
	...
	KafkaTriggerScheduler = "KafkaTrigger"
...
func NewController(ctx context.Context) *controller.Impl {
	...
	r := &Reconciler{
		Schedulers:      func(s string) scheduler.Scheduler { return schedulers[s] },
	...
	consumer.Get(ctx).Informer().AddEventHandler(controller.HandleAll(func(obj interface{}) {
		c := obj.(*kafkainternals.Consumer)
		for _, or := range c.OwnerReferences {
			if strings.ToLower(or.Kind) == strings.ToLower(kafkainternals.ConsumerGroupVersionKind.Kind) {
				impl.EnqueueKey(types.NamespacedName{Namespace: c.GetNamespace(), Name: or.Name})
				return
			}
		}
	}))
...
func createKafkaSourceScheduler(ctx context.Context, c SchedulerConfig, kafkaLister sources.KafkaSourceLister) scheduler.Scheduler {
	return createStatefulSetScheduler(
		...
		func() ([]scheduler.VPod, error) {
			sources, err := kafkaLister.List(labels.Everything())
			if err != nil {
				return nil, err
			}
