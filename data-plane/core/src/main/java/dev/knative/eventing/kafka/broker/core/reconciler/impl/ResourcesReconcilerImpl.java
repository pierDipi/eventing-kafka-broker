...
public class ResourcesReconcilerImpl implements ResourcesReconciler {
  private static final Logger logger =
    LoggerFactory.getLogger(ResourcesReconcilerImpl.class);
  ...
  // Every resource ingress is identified by its resource, so we don't need to
  // store ingress separately
  private final Map<String, DataPlaneContract.Resource> cachedResources;
  // egress uid -> <Egress, Resource>
  private final
    Map<String, Map.Entry<DataPlaneContract.Egress, DataPlaneContract.Resource>>
      cachedEgresses;

  ResourcesReconcilerImpl(IngressReconcilerListener ingressReconcilerListener,
                          EgressReconcilerListener egressReconcilerListener) {
    if (ingressReconcilerListener == null && egressReconcilerListener == null) {
      throw new NullPointerException(
        "You need to specify at least one listener");
    }
    ...
    this.cachedEgresses = new HashMap<>(
      egressReconcilerListener != null ? CACHED_RESOURCES_INITIAL_CAPACITY : 0);
  }

  @Override
  public Future<Void> reconcile(
    final Collection<DataPlaneContract.Resource> newResources) {
  ...
  private Future<Void> reconcileEgress(
    final Collection<DataPlaneContract.Resource> newResources) {
    final var egresses =
      newResources.stream()
                 ...
                 -> r.getEgressesList()
                      ...
                      .map(e
                           -> new SimpleImmutableEntry<>(
                             e.getUid(), new SimpleImmutableEntry<>(e, r))))
        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

    final List<Future> futures =
      new ArrayList<>(egresses.size() + this.cachedEgresses.size());

    final var diff =
      CollectionsUtils.diff(this.cachedEgresses.keySet(), egresses.keySet());
    ...
    diff.getRemoved().forEach(uid -> {
      ...
      futures.add(
        this.egressReconcilerListener
          ...
          .onFailure(
            cause
            -> logFailure("Failed to reconcile [onDeleteEgress] egress", egress,
                          cause)));
    });
    ...
    diff.getAdded().forEach(uid -> {
      ...
      futures.add(
        this.egressReconcilerListener
          ...
          .onSuccess(
            r
            -> this.cachedEgresses.put(
              egress.getUid(), new SimpleImmutableEntry<>(egress, resource)))
          .onFailure(cause
                     -> logFailure("Failed to reconcile [onNewEgress] egress ",
                                   egress, cause)));
    });
    ...
    diff.getIntersection().forEach(uid -> {
      ...
      if (resourceEquals(newResource, oldResource)
          && egressEquals(newEgress, this.cachedEgresses.get(uid).getKey())) {
      ...
      futures.add(
        this.egressReconcilerListener
          ...
          .onSuccess(r
                     -> this.cachedEgresses.put(
                       newEgress.getUid(),
                       new SimpleImmutableEntry<>(newEgress, newResource)))
          .onFailure(
            cause
            -> logFailure("Failed to reconcile [onUpdateEgress] egress ",
                          newEgress, cause)));
    });

    // We want to complete the future, once all futures are complete, so use
    // join.
    return CompositeFuture.join(futures)
      .onComplete(r -> {
        this.cachedResources.clear();
        egresses.values().forEach(entry -> {
          final var resource = entry.getValue();
  ...
  private Future<Void> reconcileIngress(
    Collection<DataPlaneContract.Resource> newResources) {
    final Map<String, DataPlaneContract.Resource> newResourcesMap =
      new HashMap<>(newResources.stream().collect(Collectors.toMap(
        DataPlaneContract.Resource::getUid, Function.identity())));

    final List<Future> futures =
      new ArrayList<>(newResourcesMap.size() + this.cachedResources.size());

    final var diff = CollectionsUtils.diff(this.cachedResources.keySet(),
                                           newResourcesMap.keySet());
      ...
      .forEach(
        ...
        -> futures.add(
          this.ingressReconcilerListener.onDeleteIngress(r, r.getIngress())
            ...
            .onFailure(
              cause
              -> logFailure("Failed to reconcile [onDeleteIngress] ingress", r,
                            cause))));
      ...
      .forEach(r
               -> futures.add(
                 this.ingressReconcilerListener.onNewIngress(r, r.getIngress())
                   ...
                   .onFailure(
                     cause
                     -> logFailure("Failed to reconcile [onNewIngress] ingress",
                                   r, cause))));
    ...
    diff.getIntersection().forEach(uid -> {
      ...
      futures.add(
        this.ingressReconcilerListener
          ...
          .onFailure(
            cause
            -> logFailure("Failed to reconcile [onUpdateIngress] ingress",
                          newResource, cause)));
    });
  ...
  private boolean resourceEquals(DataPlaneContract.Resource r1,
                                 DataPlaneContract.Resource r2) {
    if (r1 == r2) {
      return true;
    }
    if (r1 == null || r2 == null) {
  ...
  private boolean egressEquals(DataPlaneContract.Egress e1,
                               DataPlaneContract.Egress e2) {
    if (e1 == e2) {
      return true;
    }
    if (e1 == null || e2 == null) {
    ...
    return Objects.equals(e1.getUid(), e2.getUid())
      ...
      && Objects.equals(e1.getReplyToOriginalTopic(),
                        e2.getReplyToOriginalTopic())
  ...
  private static void logFailure(final String msg,
                                 final DataPlaneContract.Egress egress,
                                 final Throwable cause) {
    logger.error(msg + " {} {} {}", keyValue("id", egress.getUid()),
                 keyValue("consumerGroup", egress.getConsumerGroup()),
                 keyValue("destination", egress.getDestination()), cause);
  }

  private static void logFailure(final String msg,
                                 final DataPlaneContract.Resource resource,
                                 final Throwable cause) {
    logger.error(msg + " {} {} {}", keyValue("id", resource.getUid()),
                 keyValue("ingress.path", resource.getIngress().getPath()),
                 keyValue("topics", resource.getTopicsList()), cause);
  }
}
