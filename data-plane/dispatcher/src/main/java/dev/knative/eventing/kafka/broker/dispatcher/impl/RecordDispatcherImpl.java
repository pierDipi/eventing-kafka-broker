...
public class RecordDispatcherImpl implements RecordDispatcher {
  private static final Logger logger =
    LoggerFactory.getLogger(RecordDispatcherImpl.class);

  private final Filter filter;
  private final Function<KafkaConsumerRecord<Object, CloudEvent>, Future<Void>>
    subscriberSender;
  private final Function<KafkaConsumerRecord<Object, CloudEvent>, Future<Void>>
    dlsSender;
   ...
   * @param responseHandler          handler of the response from {@code
   *   subscriberSender}
   * @param recordDispatcherListener hook receiver {@link
   *   RecordDispatcherListener}. It allows to plug in custom offset
   * @param consumerTracer           consumer tracer
   */
  public RecordDispatcherImpl(
    final Filter filter, final CloudEventSender subscriberSender,
    ...
    Objects.requireNonNull(deadLetterSinkSender,
                           "provide deadLetterSinkSender");
    ...
    this.filter = filter;
    this.subscriberSender = composeSenderAndSinkHandler(
      subscriberSender, responseHandler, "subscriber");
    this.dlsSender = composeSenderAndSinkHandler(
      deadLetterSinkSender, responseHandler, "dead letter sink");
    this.recordDispatcherListener = recordDispatcherListener;
    this.closeable = AsyncCloseable.compose(
      responseHandler, deadLetterSinkSender, subscriberSender);
    this.consumerTracer = consumerTracer;
  }

  /**
   * Handle the given record and returns a future that completes when the
   * dispatch is completed and the offset is committed
   *
  ...
  private void onRecordReceived(
    final KafkaConsumerRecord<Object, CloudEvent> record,
    Promise<Void> finalProm) {
    logDebug("Handling record", record);

    // Trace record received event
    // This needs to be done manually in order to work properly with both
    // ordered and unordered delivery.
    if (consumerTracer != null) {
      Context context = Vertx.currentContext();
      ConsumerTracer.StartedSpan span =
        consumerTracer.prepareMessageReceived(context, record.record());
      if (span != null) {
  ...
  private void onFilterNotMatching(
    final KafkaConsumerRecord<Object, CloudEvent> record,
    final Promise<Void> finalProm) {
    logDebug("Record doesn't match filtering", record);
    recordDispatcherListener.recordDiscarded(record).onComplete(finalProm);
  }
  ...
  private void onSubscriberSuccess(
    ...
    recordDispatcherListener.successfullySentToSubscriber(record).onComplete(
      finalProm);
  }
  ...
  private void onDeadLetterSinkSuccess(
    final KafkaConsumerRecord<Object, CloudEvent> record,
    final Promise<Void> finalProm) {
  ...
  private void onDeadLetterSinkFailure(
    final KafkaConsumerRecord<Object, CloudEvent> record,
    final Throwable exception, final Promise<Void> finalProm) {
    recordDispatcherListener.failedToSendToDeadLetterSink(record, exception)
      .onComplete(finalProm);
  }

  private static Function<KafkaConsumerRecord<Object, CloudEvent>, Future<Void>>
  composeSenderAndSinkHandler(CloudEventSender sender,
                              ResponseHandler sinkHandler, String senderType) {
    return rec
      -> sender.send(rec.value())
           .onFailure(
             ex -> logError("Failed to send event to " + senderType, rec, ex))
           .compose(
             res
             -> sinkHandler.handle(res).onFailure(
               ex
               -> logError("Failed to handle " + senderType + " response", rec,
                           ex)));
  }

  private static void logError(
    final String msg, final KafkaConsumerRecord<Object, CloudEvent> record,
    final Throwable cause) {
    if (logger.isDebugEnabled()) {
      logger.error(msg + " {} {} {} {} {}", keyValue("topic", record.topic()),
                   keyValue("partition", record.partition()),
                   keyValue("headers", record.headers()),
                   keyValue("offset", record.offset()),
                   keyValue("event", record.value()), cause);
    } else {
      logger.error(msg + " {} {} {}", keyValue("topic", record.topic()),
                   keyValue("partition", record.partition()),
                   keyValue("offset", record.offset()), cause);
    }
  }

  private static void logDebug(
    final String msg, final KafkaConsumerRecord<Object, CloudEvent> record) {
    logger.debug(msg + " {} {} {} {} {} {}", keyValue("topic", record.topic()),
                 keyValue("partition", record.partition()),
                 keyValue("headers", record.headers()),
                 keyValue("offset", record.offset()),
