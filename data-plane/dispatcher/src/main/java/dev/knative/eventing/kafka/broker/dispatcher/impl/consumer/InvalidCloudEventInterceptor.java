 ...
 * <p>
 * This interceptor is capable of building a valid {@link CloudEvent} from a
 * message that doesn't follow the Kafka protocol binding for CloudEvents. <p>
 * The {@link CloudEventDeserializer} wraps invalid {@link CloudEvent}s in an
 * instance of {@link InvalidCloudEvent} so that this interceptor can build a
 * valid {@link CloudEvent} from {@link ConsumerRecord}'s metadata and data. <p>
 * For this reason this interceptor is capable of creating a CloudEvent from
 * {@link ConsumerRecord} metadata and data.
 */
public class InvalidCloudEventInterceptor
  implements ConsumerInterceptor<Object, CloudEvent> {
  private static final Logger logger =
    LoggerFactory.getLogger(InvalidCloudEventInterceptor.class);

  public static final String KIND_PLURAL_CONFIG =
    "cloudevent.invalid.kind.plural";
  public static final String SOURCE_NAMESPACE_CONFIG =
    "cloudevent.invalid.source.namespace";
  public static final String SOURCE_NAME_CONFIG =
    "cloudevent.invalid.source.name";
  ...
  public void configure(final Map<String, ?> configs) {
    ...
    if (configs.containsKey(
          CloudEventDeserializer.INVALID_CE_WRAPPER_ENABLED)) {
      final var enabled =
        configs.get(CloudEventDeserializer.INVALID_CE_WRAPPER_ENABLED);
      if (enabled != null) {
    ...
    isEnabled = isEnabled && isNotBlank(kindPlural) && isNotBlank(sourceName)
  ...
  @Override
  public ConsumerRecords<Object, CloudEvent> onConsume(
    final ConsumerRecords<Object, CloudEvent> records) {
    if (!this.isEnabled || records == null || records.isEmpty()) {
      return records;
    }

    final Map<TopicPartition, List<ConsumerRecord<Object, CloudEvent>>>
      validRecords = new HashMap<>(records.count());
  ...
  private static String getOrDefault(final Map<String, ?> configs,
                                     final String key) {
  ...
  private ConsumerRecord<Object, CloudEvent> validRecord(
    ...
    // Create CloudEvent from the record metadata (topic, partition, offset,
    // etc) and use received data as data field. The record to CloudEvent
    // conversion is compatible with eventing-kafka.
    ...
    record.headers().forEach(
      header
      -> value.withExtension(replaceBadChars(header.key()), header.value()));
    ...
    return new ConsumerRecord<>(
      ...
      record.serializedValueSize(), record.key(), value.build(),
  ...
  private static String replaceBadChars(String value) {
    ...
    final Stream<Character> validChars =
      value
        .chars()
  ...
  private URI source(final ConsumerRecord<Object, CloudEvent> record) {
    return URI.create("/apis/v1/namespaces/" + this.sourceNamespace + "/"
                      + this.kindPlural + "/" + this.sourceName + "#"
                      + record.topic());
  }
  ...
  private static OffsetDateTime time(
    final ConsumerRecord<Object, CloudEvent> record) {
    return OffsetDateTime.ofInstant(Instant.ofEpochMilli(record.timestamp()),
                                    ZoneId.of("UTC"));
  }
