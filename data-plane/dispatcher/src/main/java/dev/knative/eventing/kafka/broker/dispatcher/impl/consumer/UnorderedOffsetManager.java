...
/**
 * This class implements the offset strategy that makes sure that, even
 * unordered, the offset commit is ordered.
 */
public final class UnorderedOffsetManager implements RecordDispatcherListener {
  private static final Logger logger =
    LoggerFactory.getLogger(UnorderedOffsetManager.class);
   ...
   */
  public UnorderedOffsetManager(final KafkaConsumer<?, ?> consumer,
  ...
  public Future<Void> recordReceived(final KafkaConsumerRecord<?, ?> record) {
    ...
    // TODO If this assumption doesn't work, use this.consumer.committed(new
    // TopicPartition(record.topic(), record.partition()))
    this.offsetTrackers.computeIfAbsent(
  ...
  private Future<Void> commit(final KafkaConsumerRecord<?, ?> record) {
    ...
    if (tracker.shouldCommit()) {
      ...
      return consumer
        .commit(Map.of(topicPartition, new OffsetAndMetadata(newOffset, "")))
        .onSuccess(ignored -> {
          if (onCommit != null) {
            onCommit.accept((int)uncommittedSize);
          }
          logger.debug("committed for topic partition {} {} offset {}",
                       record.topic(), record.partition(), newOffset);
        })
        .onFailure(cause
                   -> logger.error(
                     "failed to commit for topic partition {} {} offset {}",
                     record.topic(), record.partition(), newOffset, cause))
        .mapEmpty();
    }
    return Future.succeededFuture();
  }
   ...
   *
   * <p>
   * For each record, the tracker flip a bit at an index representing the
   * difference between the record's offset and the last acked record offset. To
   * figure out if we need to commit or not, we just need to check if every bit
   * of the long, up to the greatest uncommitted offset, is 1. In numerical
   * representation this is equal to 2<sup>(greatestIndex - lastAcked)</sup>
   * - 1. To avoid computing these numbers, we statically initialize all masks.
   *
   * <p>
   * Because a long is only 64 bit, we use an array of longs (blocks) to
   * represent the eventual uncommitted records. If blocks are more than one, in
   * order to commit, every block except the last one must have all bits flipped
   * to 1 (in numerical representation -1 because of 2-complement representation
   * of long) and the last block should follow the rule above explained. Note:
   * in the best and more realistic case the store needs just 1 long, which
   * means that only 64 records are sent unordered.
   */
  private static class OffsetTracker {
    private final static int ACKS_GARBAGE_SIZE_THRESHOLD =
      16;  // Meaning 1024 messages are on hold
    ...
    public void recordNewOffset(long offset) {
      ...
      int bitIndex = (int)(diffWithLastCommittedOffset
                           % 64);  // That's obviously smaller than a long
      ...
      } else if (this.greaterBlockIndex == blockIndex
                 && this.greaterBitIndexInGreaterBlock < bitIndex) {
        this.greaterBitIndexInGreaterBlock = bitIndex;
      }
    }
    ...
    public boolean shouldCommit() {
      ...
      return this.uncommitted[this.greaterBlockIndex]
        == MASKS[this.greaterBitIndexInGreaterBlock];
    }
    ...
    private void checkAcksArraySize(int blockIndex) {
      if (blockIndex > this.uncommitted.length - 1) {
        ...
        this.uncommitted =
          Arrays.copyOf(this.uncommitted, (blockIndex + 1) * 2);
      }
    }
  }
}
