...
/**
 * This verticle listens on Egress reconciliations by deploying/undeploying new
 * consumer verticles.
 */
public final class ConsumerDeployerVerticle
  extends AbstractVerticle implements EgressReconcilerListener {
  private static final Logger logger =
    LoggerFactory.getLogger(ConsumerDeployerVerticle.class);
   ...
   */
  public ConsumerDeployerVerticle(final ConsumerVerticleFactory consumerFactory,
    ...
    if (egressesInitialCapacity <= 0) {
      throw new IllegalArgumentException(
        "egressesInitialCapacity cannot be negative or 0");
    }
  ...
  public void start() {
    this.messageConsumer =
      ResourcesReconciler.builder().watchEgress(this).buildAndListen(vertx);
  }
  ...
  @Override
  public Future<Void> onNewEgress(final DataPlaneContract.Resource resource,
    ...
    try {
      ...
      return vertx.deployVerticle(verticle)
        ...
        .onFailure(cause -> {
          // this is a bad state we cannot start the verticle for consuming
          // messages.
          logger.error("failed to start verticle {} {}",
                       keyValue("egress", egress),
                       keyValue("resource", resource), cause);
        })
        .mapEmpty();
    } catch (Exception e) {
      logger.error("Potential control-plane bug: failed to get verticle {} {}",
                   keyValue("egress.uid", egress.getUid()),
                   keyValue("resource.uid", resource.getUid()), e);
      return Future.failedFuture(new IllegalStateException(
        "Potential control-plane bug: failed to get verticle", e));
    }
  }

  @Override
  public Future<Void> onUpdateEgress(final DataPlaneContract.Resource resource,
                                     final DataPlaneContract.Egress egress) {
  ...
  @Override
  public Future<Void> onDeleteEgress(final DataPlaneContract.Resource resource,
                                     final DataPlaneContract.Egress egress) {
    if (!this.deployedDispatchers.containsKey(egress.getUid())) {
      return Future.succeededFuture();
    }
    ...
    return vertx.undeploy(this.deployedDispatchers.get(egress.getUid()))
      .compose(
        ...
        cause -> {
          // IllegalStateException is thrown when a verticle is already
          // un-deployed.
          ...
          logger.error("Failed to un-deploy verticle {} {}",
                       keyValue("egress.uid", egress.getUid()),
                       keyValue("resource.uid", resource.getUid()), cause);
