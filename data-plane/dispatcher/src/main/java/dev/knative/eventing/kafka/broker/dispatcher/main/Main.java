...
public class Main {
  ...
  public static void main(final String[] args) throws IOException {
    ...
    Properties producerConfig =
      Configurations.readPropertiesSync(env.getProducerConfigFilePath());
    ...
    producerConfig.put(ProducerConfig.INTERCEPTOR_CLASSES_CONFIG,
                       PartitionKeyExtensionInterceptor.class.getName());
    Properties consumerConfig =
      Configurations.readPropertiesSync(env.getConsumerConfigFilePath());
    consumerConfig.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG,
                       KeyDeserializer.class.getName());
    consumerConfig.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG,
                       CloudEventDeserializer.class.getName());
    consumerConfig.put(ConsumerConfig.INTERCEPTOR_CLASSES_CONFIG,
                       InvalidCloudEventInterceptor.class.getName());

    // Read WebClient config
    JsonObject webClientConfig =
      Configurations.readPropertiesAsJsonSync(env.getWebClientConfigFilePath());
    ...
    try {
      ...
      ConsumerDeployerVerticle consumerDeployerVerticle =
        new ConsumerDeployerVerticle(
          new ConsumerVerticleFactoryImpl(
            consumerConfig, webClientOptions, producerConfig,
            AuthProvider.kubernetes(), Metrics.getRegistry()),
      ...
      FileWatcher fileWatcher =
        new FileWatcher(new File(env.getDataPlaneConfigFilePath()), publisher);
      fileWatcher.start();

      // Register shutdown hook for graceful shutdown.
      Shutdown.registerHook(vertx, publisher, fileWatcher,
