...
public abstract class AbstractOffsetManagerTest {
  ...
  createOffsetManager(final KafkaConsumer<?, ?> consumer);

  protected static KafkaConsumerRecord<String, CloudEvent> record(String topic,
                                                                  int partition,
                                                                  long offset) {
  ...
  assertThatOffsetCommittedWithFailures(
    ...
    final KafkaConsumer<String, CloudEvent> vertxConsumer =
      mock(KafkaConsumer.class);
    doAnswer(invocation -> {
      if (failureFlag.get()) {
        return Future.failedFuture("some failure");
      }
      // If you don't want to lose hours in debugging, please don't remove this
      // FQCNs :)
      ...
      mockConsumer.commitSync(
        topicsPartitions.entrySet().stream().collect(Collectors.toMap(
          e
          -> new TopicPartition(e.getKey().getTopic(),
    ...
    return assertThat(mockConsumer.committed(Set.copyOf(partitionsConsumed))
                        .entrySet()
                        .stream()
  ...
  protected void shouldNeverCommit(
  ...
  @SuppressWarnings("unchecked")
  protected void shouldNeverPause(
    final KafkaConsumer<String, CloudEvent> consumer) {
    verify(consumer, never()).pause();
    verify(consumer, never())
      .pause(any(io.vertx.kafka.client.common.TopicPartition.class));
    verify(consumer, never()).pause(any(Set.class));
    verify(consumer, never())
      .pause(any(io.vertx.kafka.client.common.TopicPartition.class), any());
