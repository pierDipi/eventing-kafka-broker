...
public class OrderedConsumerVerticleTest extends AbstractConsumerVerticleTest {
  private static Stream<Arguments> inputArgs() {
    return Stream.of(
      Arguments.of(0L, 10, 10, true), Arguments.of(0L, 100, 10, true),
      Arguments.of(0L, 1_000, 10, true), Arguments.of(0L, 10_000, 10, true),
      Arguments.of(50L, 500, 10, true), Arguments.of(50L, 100_000, 1_000, true),
      Arguments.of(50L, 100_000, 1_000, false),
      Arguments.of(100L, 10, 10, true), Arguments.of(100L, 100, 10, true),
  ...
  @ParameterizedTest(
    name =
      "with delay {0}ms, {1} tasks, {2} partitions and random partition assignment {3}")
  @MethodSource("inputArgs")
  public void
  consumeOneByOne(final long delay, final int tasks, final int partitions,
                  final boolean randomAssignment, final Vertx vertx)
    throws InterruptedException {
    final var topic = "topic1";
    final Random random = new Random();
    final var consumer =
      new MockConsumer<Object, CloudEvent>(OffsetResetStrategy.LATEST);

    // Mock the record dispatcher to count down the latch and save the received
    // records order
    CountDownLatch latch = new CountDownLatch(tasks);
    final Map<TopicPartition, List<Long>> receivedRecords =
    ...
    when(recordDispatcher.dispatch(any())).then(invocation -> {
      ...
      return recordDispatcherLogicMock(vertx, random, delay, latch, record,
                                       receivedRecords);
    });
    when(recordDispatcher.close()).thenReturn(Future.succeededFuture());

    final var verticle = createConsumerVerticle((vx, consumerVerticle) -> {
    ...
    assertThat(receivedRecords.values().stream().mapToInt(List::size).sum())
      .isEqualTo(tasks);
  }
  ...
  protected static ConsumerRecord<Object, CloudEvent> record(String topic,
                                                             int partition,
                                                             long offset) {
  ...
  private Future<Void> recordDispatcherLogicMock(
    Vertx vertx, Random random, long millis, CountDownLatch latch,
    KafkaConsumerRecord<String, CloudEvent> record,
    Map<TopicPartition, List<Long>> receivedRecords) {
    if (millis == 0) {
      receivedRecords
        .computeIfAbsent(new TopicPartition(record.topic(), record.partition()),
                         tp -> new ArrayList<>())
        .add(record.offset());
      latch.countDown();
      return Future.succeededFuture();
      ...
      long delay =
        Math.round(Math.ceil(millis + (random.nextDouble() * millis * 0.5)));
