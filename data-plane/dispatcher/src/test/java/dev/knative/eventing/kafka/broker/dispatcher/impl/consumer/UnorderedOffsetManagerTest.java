...
public class UnorderedOffsetManagerTest extends AbstractOffsetManagerTest {
  ...
  @Test
  public void
  shouldCommitAfterSendingEventsOrderedOnTheSamePartitionWithInducedFailure() {
    assertThatOffsetCommittedWithFailures(
      List.of(new TopicPartition("aaa", 0)),
      (offsetStrategy, failureFlag) -> {
        offsetStrategy.recordReceived(record("aaa", 0, 0));
  ...
  public void shouldCommitAfterSendingEventsOrderedOnDifferentPartitions() {
    assertThatOffsetCommitted(
      List.of(new TopicPartition("aaa", 0), new TopicPartition("aaa", 1)),
      offsetStrategy -> {
        offsetStrategy.recordReceived(record("aaa", 0, 0));
    ...
    assertThatOffsetCommitted(
      ...
      offsetStrategy -> {
        ...
        List.of(5L, 2L, 0L, 7L, 1L, 3L, 4L, 6L)
          .forEach(offset
                   -> offsetStrategy.successfullySentToSubscriber(
                     record("aaa", 0, offset)));
      })
  ...
  @Test
  public void
  shouldCommitSuccessfullyWithRecordFailedToDeadLetterSinkInTheMiddle() {
    assertThatOffsetCommitted(
      List.of(new TopicPartition("aaa", 0)),
      offsetStrategy -> {
        var rec = record("aaa", 0, 0);
