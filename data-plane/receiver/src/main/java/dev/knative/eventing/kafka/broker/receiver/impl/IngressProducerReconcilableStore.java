...
/**
 * This class implements a store of {@link IngressProducer} that can be
 * reconciled using it as {@link IngressReconcilerListener} of a {@link
 * dev.knative.eventing.kafka.broker.core.reconciler.ResourcesReconciler}. <p>
 * Because of its state associated to the specific verticles, this class cannot
 * be shared among verticles.
 */
public class IngressProducerReconcilableStore
  implements IngressReconcilerListener {
  private final Properties producerConfigs;
  private final Function<Properties, KafkaProducer<String, CloudEvent>>
    producerFactory;
  ...
  // This map is used to count the references to the producer instantiated for
  // each producerConfig
  private final Map<Properties, ReferenceCounter<ProducerHolder>>
    producerReferences;
  // path -> IngressInfo
  // We use this map on the hot path to directly resolve the producer from the
  // path
  private final Map<String, IngressProducerImpl> pathMapper;

  public IngressProducerReconcilableStore(
    final AuthProvider authProvider, final Properties producerConfigs,
    final Function<Properties, KafkaProducer<String, CloudEvent>>
      producerFactory) {
  ...
  @Override
  public Future<Void> onNewIngress(DataPlaneContract.Resource resource,
    ...
    if (resource.hasAuthSecret()) {
      return authProvider
        .getCredentials(resource.getAuthSecret().getNamespace(),
                        resource.getAuthSecret().getName())
        .map(credentials
             -> KafkaClientsAuth.attachCredentials(producerProps, credentials))
  ...
  private Future<Void> onNewIngress(final DataPlaneContract.Resource resource,
    ...
    if (ingress.getContentMode()
        != DataPlaneContract.ContentMode.UNRECOGNIZED) {
      producerProps.setProperty(CloudEventSerializer.ENCODING_CONFIG,
                                encoding(ingress.getContentMode()));
    }
    producerProps.setProperty(CloudEventSerializer.EVENT_FORMAT_CONFIG,
    ...
    try {
      ...
      final var ingressInfo = new IngressProducerImpl(
        rc.getValue().getProducer(), resource.getTopics(0), ingress.getPath(),
        producerProps);
  ...
  @Override
  public Future<Void> onUpdateIngress(DataPlaneContract.Resource resource,
                                      DataPlaneContract.Ingress ingress) {
    // TODO this update can produce errors when onDeleteIngress finishes and
    // before onNewIngress creates mappings.
  ...
  public Future<Void> onDeleteIngress(final DataPlaneContract.Resource resource,
    ...
    // Get the rc
    final var rc =
      this.producerReferences.get(ingressInfo.getProducerProperties());
  ...
  private static String encoding(
    final DataPlaneContract.ContentMode contentMode) {
