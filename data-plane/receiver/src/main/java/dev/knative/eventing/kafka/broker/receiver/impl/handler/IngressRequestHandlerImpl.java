...
/**
 * Implementation of {@link IngressRequestHandler} that will produce incoming
 * requests using the provided producer. <p> Instances of this class can be
 * shared among verticles, given the {@code requestToRecordMapper} provided is
 * shareable among verticles.
 */
public class IngressRequestHandlerImpl implements IngressRequestHandler {
  ...
  private static final Logger logger =
    LoggerFactory.getLogger(IngressRequestHandlerImpl.class);
  ...
  public IngressRequestHandlerImpl(RequestToRecordMapper requestToRecordMapper,
  ...
  public void handle(HttpServerRequest request, IngressProducer producer) {
    requestToRecordMapper.requestToRecord(request, producer.getTopic())
      .onFailure(cause -> {
        ...
        logger.warn("Failed to convert request to record {}",
                    keyValue("path", request.path()), cause);
      })
      .compose(record -> {
        ...
        if (logger.isDebugEnabled()) {
          ...
          if (span != null) {
            logger.debug("Received event {} {}",
                         ...
                         keyValue(TracingConfig.TRACE_ID_KEY,
  ...
  private Future<RecordMetadata> publishRecord(
    IngressProducer ingress, KafkaProducerRecord<String, CloudEvent> record) {
    return ingress.send(record).onComplete(ar -> {
      if (ar.succeeded()) {
        ...
        logger.error("Failed to send record {} {}",
                     keyValue("topic", record.topic()), ar.cause());
      }
    });
  }
}
