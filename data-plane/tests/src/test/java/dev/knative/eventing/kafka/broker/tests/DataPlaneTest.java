...
public class DataPlaneTest {
  ...
  public void execute(final Vertx vertx, final VertxTestContext context)
      ...
      DataPlaneContract.Resource.newBuilder()
        ...
        .addEgresses(
          DataPlaneContract.Egress.newBuilder()
            .setUid(UUID.randomUUID().toString())
            .setDestination(
              format("http://localhost:%d%s", SERVICE_PORT, PATH_SERVICE_1))
            .setFilter(DataPlaneContract.Filter.newBuilder().putAttributes(
              CloudEventV1.TYPE, TYPE_CE_1))
        ...
        .addEgresses(
          DataPlaneContract.Egress.newBuilder()
            .setUid(UUID.randomUUID().toString())
            .setDestination(
              format("http://localhost:%d%s", SERVICE_PORT, PATH_SERVICE_2))
            .setFilter(DataPlaneContract.Filter.newBuilder().putAttributes(
              CloudEventV1.TYPE, TYPE_CE_2))
            .setConsumerGroup(UUID.randomUUID().toString())
            .build())
        .addEgresses(
          // the destination of the following trigger should never be reached
          // because events don't pass filters.
          DataPlaneContract.Egress.newBuilder()
            .setUid(UUID.randomUUID().toString())
            .setConsumerGroup(UUID.randomUUID().toString())
            .setDestination(
              format("http://localhost:%d%s", SERVICE_PORT, PATH_SERVICE_3))
            .setFilter(DataPlaneContract.Filter.newBuilder().putAttributes(
              CloudEventV1.SOURCE, UUID.randomUUID().toString()))
            .build())
        .build();
    ...
    await()
      ...
      .untilAsserted(()
                       -> assertThat(vertx.deploymentIDs())
                            .hasSize(resource.getEgressesCount() + NUM_RESOURCES
                                     + NUM_SYSTEM_VERTICLES));
    ...
    vertx.createHttpServer()
        ...
        -> VertxMessageFactory.createReader(request)
             ...
             .onSuccess(event -> {
               ...
               if (request.path().equals(PATH_SERVICE_1)) {
                 ...
                 // write event to the response, the event will be handled by
                 // service 2
      ...
      .onSuccess(ignored -> {
        ...
        VertxMessageFactory
          .createWriter(WebClient.create(vertx).post(
            INGRESS_PORT, "localhost",
            format("/%s/%s", BROKER_NAMESPACE, BROKER_NAME)))
  ...
  private static ConsumerDeployerVerticle setUpDispatcher(
    ...
    consumerConfigs.put(BOOTSTRAP_SERVERS_CONFIG,
                        format("localhost:%d", KAFKA_PORT));
    consumerConfigs.put(KEY_DESERIALIZER_CLASS_CONFIG,
                        StringDeserializer.class.getName());
    consumerConfigs.put(VALUE_DESERIALIZER_CLASS_CONFIG,
    ...
    final var consumerVerticleFactory = new ConsumerVerticleFactoryImpl(
      consumerConfigs, new WebClientOptions(), producerConfigs,
      mock(AuthProvider.class), Metrics.getRegistry());
  ...
  private static ReceiverVerticle setUpReceiver(final Vertx vertx,
    ...
    final var verticle = new ReceiverVerticle(
      ...
      v
      -> new IngressProducerReconcilableStore(
        null, producerConfigs(),
  ...
  private static Properties producerConfigs() {
    ...
    configs.put(VALUE_SERIALIZER_CLASS_CONFIG,
