...
EOF
  ...
  git clone --branch release-1.27 https://github.com/openshift-knative/serverless-operator.git $operator_dir
...
#  fi
  go_test_e2e -timeout=100m -run TestKafkaSourceDeletedFromContractConfigMaps ./test/e2e_new/... || return $?
